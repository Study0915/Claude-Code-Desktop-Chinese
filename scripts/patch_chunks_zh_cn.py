...
def take_ownership(path: Path) -> bool:
    """Take ownership of a file or directory using takeown + icacls (WindowsApps fix)."""
    try:
        target = path if path.exists() else path.parent
        if not target.exists():
            print(f"Warning: cannot take ownership, path does not exist: {target}")
            return False

        print(f"  Fixing permissions on: {target}")
        ...
        if r1.returncode != 0:
            err = r1.stderr.decode("utf-8", errors="ignore").strip() or r1.stdout.decode("utf-8", errors="ignore").strip()
            print(f"    takeown warning: {err}")
        ...
        if r2.returncode != 0:
            err = r2.stderr.decode("utf-8", errors="ignore").strip() or r2.stdout.decode("utf-8", errors="ignore").strip()
            print(f"    icacls warning: {err}")

        return True
    except Exception as e:
        print(f"Warning: takeown/icacls failed on {path}: {e}")
        return False


def copy2_best_effort(src: Path, dst: Path, *, context: str) -> bool:
    """Copy a file, attempting takeown on PermissionError for WindowsApps."""
    try:
...
# === Universal navigation / sidebar labels (applied to ALL *.js files) ===
# These are generic UI labels that may move between chunks across versions.
UNIVERSAL_PATCHES: list[tuple[str, str]] = [
    # Tab labels
    ('label:"Chat"', 'label:"聊天"'),
    ...
    # Section headers / pinned / recents
...
# === Sidebar navigation (applied to all chunk files) ===
# NOTE: Most sidebar labels have been moved to UNIVERSAL_PATCHES above.
# The legacy per-file patches below are kept for backwards compatibility
# with older Claude versions where the exact chunk filename is still used.
...
def main() -> int:
    ...
    # Pre-emptively fix permissions on the entire assets directory so we
    # don't hit PermissionError on every individual file write inside WindowsApps.
    print("Fixing permissions on assets directory (recursive)...")
    try:
        r1 = subprocess.run(
            ["takeown", "/F", str(assets_dir), "/A", "/R", "/D", "Y"],
            capture_output=True, timeout=120
        )
        r2 = subprocess.run(
            ["icacls", str(assets_dir), "/grant", "Administrators:F", "/T"],
            capture_output=True, timeout=120
        )
        if r1.returncode != 0:
            err = r1.stderr.decode("utf-8", errors="ignore").strip() or r1.stdout.decode("utf-8", errors="ignore").strip()
        ...
        print(f"  Warning: could not fix permissions on assets dir: {e}")

    total = 0

    # Step 1: Apply universal patches to ALL *.js files
    # This ensures navigation labels are patched even when chunk filenames
    # change across Claude Desktop versions.
    if UNIVERSAL_PATCHES:
        all_js_files = sorted(assets_dir.glob("*.js"))
        print(f"Applying universal patches to {len(all_js_files)} JS files...")
        for fpath in all_js_files:
            backup_file(fpath, assets_dir)
            content = fpath.read_text(encoding="utf-8")
            changed = 0
            for old, new in UNIVERSAL_PATCHES:
                if old in content and old != new:
                    content = content.replace(old, new)
                    changed += 1
            if changed > 0:
                if write_text_best_effort(fpath, content, context="universal chunk replacement"):
                    total += changed
                    print(f"  {fpath.name}: {changed} universal replacements")

    # Step 2: Apply per-file specific patches (legacy / special cases)
