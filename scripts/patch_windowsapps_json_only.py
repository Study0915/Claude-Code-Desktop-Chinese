...
def take_ownership(path: Path) -> bool:
    ...
    try:
        # If path doesn't exist (e.g. a new file we want to create), operate on its parent directory
...
def copy2_best_effort(src: Path, dst: Path, *, context: str) -> bool:
    ...
    try:
        ...
        # Always attempt permission fix, even for new files (fixes parent dir ACL)
        take_ownership(dst)
        try:
            shutil.copy2(src, dst)
            return True
